<template>
  <div class="map_box">
	...
	<div style="width: 100%;height: 100%;position: absolute;top: 0;left:0;background:transparent;">
		<iframe ref="mapCn" id="mapCn" src="static/3dMap/map_cn_new.html?type=6" width="100%" height="100%" scrolling="no" frameborder="0"></iframe>
	</div>
	
	
	
    <div v-show="reFresh" @click="goToproduct" class="topology animated fadeInUp">
      <h3>{{topoInfo.name}}</h3>
      <!-- 成都道桥 -->
      <!-- <iframe v-if="active=='1'" src="static/case/jyhy_max_1/index.html" width="100%" height="84%" scrolling="no" frameborder="0"></iframe> -->
      <iframe  style="display:block;" :src="`static/case/${topoInfo.caseTopMap}/index.html`" width="100%" height="84%" scrolling="no" frameborder="0"></iframe>
    </div>
    <div class="mix animated fadeInLeft">
      <h2><img class="logosnnsns" src="../../assets/index/网管信息.png" alt="" srcset=""> <label style="vertical-align: middle;">网管信息</label><span>Network Management Message</span></h2>
        ...
        <div class="mix_box " v-for="(item,i) in proportion" :key="i">
          ...
          <div class="nb">
            <div class="nb1" v-for="(key,index) in item.info" :key="index">
              <p style="color: #fff"><span :class="(key.name!='机房编号' && key.name!='机房地点' && key.name!='机柜数量' && key.name!='服务器编号')?'val4 val4map':'val456423123'" :data-target="key.nub">{{(key.name!='机房编号' && key.name!='机房地点' && key.name!='机柜数量' && key.name!='服务器编号')?0:key.nub}}</span><span class="bfb">{{key.unit}}</span></p>
              <span style="color: #40dcf8">{{key.name}}</span>
              <div v-if="i>0" class="bar">
                <div class="bar_color val4_bar val4_barmap" :data-target="key.nub"></div>
              </div>
            </div>
          </div>
...
export default {
  ...
  watch:{
    ...
    websockData(val) {
      console.log('Map地图',val)
      if(val && !!val.caseInfoItem) {
        this.reFresh = false;
        this.topoInfo = val.caseInfoItem;
        setTimeout(() => {
          this.reFresh = true;
        }, 500);
      }
    ...
    productseleted: {
      handler(val) {
        if(!!val) {
          console.log('调了没')
          //this.$refs.mapCn.contentWindow.newInit(6)
        }
      },
      immediate: true,
      // deep:true
  ...
  mounted() {
	  window.getOrderInfo = this.proportionUpdate;
  ...
  methods: {
    animateAlldou() {
      let val4map = document.getElementsByClassName("val4map");
      let math = parseFloat(Math.random()*(24-23)+23);
      let math1 = parseFloat(Math.random()*(41-40)+40);
      this.animateNum(val4map[0], math, 1);
      this.animateNum(val4map[1], math1, 1);
    },
    animateAll() {
      let val4map = document.getElementsByClassName("val4map");
      for (let i = 0; i < val4map.length; i++) {
        let targetDomCount = val4map[i].getAttribute("data-target");
        this.animateNum(val4map[i], targetDomCount, 1);
    ...
    getMapInit(){
      let t = this;
      console.log('**************************************调用地图方法',this.$refs.mapCn.contentWindow.newInit)
      setTimeout(()=>{
        if(this.$refs.mapCn.contentWindow.caseInitFun){
			
		  this.$refs.mapCn.contentWindow.removeConnection()
		  this.$refs.mapCn.contentWindow.caseInitFun(6,true)
        } else {
          t.getMapInit()
        }
      },500)
